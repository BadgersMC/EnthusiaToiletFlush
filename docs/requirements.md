...
- REQ-012: The system shall send the backend restart signal only after the target is empty. After the final transfer batch has had one batch interval to settle, it shall issue configured disconnects to any players still on the target and wait for Velocity to observe the target empty; if `force-drain-timeout-seconds` elapses first, it shall issue those disconnects and send the immediate restart as a safety fallback.
